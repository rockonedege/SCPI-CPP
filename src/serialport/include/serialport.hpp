 ...
 */
class timed_serial_port : private boost::noncopyable
{
public:
    timed_serial_port();
     ...
     * serial device
     */
    timed_serial_port(const std::string &devname, unsigned int baud_rate,
...
private:
     ...
     * If called because timeout expired, sets result to READ_TIMEOUT
     */
    void timeoutExpired(const boost::system::error_code &error);

    /**
     * Callback called either if a read complete or read error occurs
     * If called because of read complete, sets result to READ_SUCCESS
     * If called because read error, sets result to READ_ERROR
     */
    ...
    boost::asio::io_context m_io;                          ///< Io service object
    boost::asio::serial_port m_port;                       ///< Serial port object
    boost::asio::steady_timer m_timer;                     ///< Timer for timeout
    boost::asio::chrono::steady_clock::duration m_timeout; ///< Read/write timeout
    boost::asio::streambuf m_readData;                     ///< Holds eventual read but not consumed
    READ_RESULT m_readresult;                                  ///< Used by read with timeout
    size_t m_bytesTransferred;                             ///< Used by async read callback
    ReadSetupParameters m_setupParameters;                 ///< Global because used in the OSX fix
};
